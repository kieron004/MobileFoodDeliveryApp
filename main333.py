...
# main.py
from FutureOrdersWishlist import FutureOrdersWishlist  # 导入愿望清单类
...
class MainAppFrame(tk.Frame):
    def __init__(self, master, user_email):
        ...
        # Initialize RestaurantFilter with a list of restaurants
        restaurants_data = [
            {"name": "Pasta House", "cuisine": "Italian", "rating": 4.5, "delivery_time": 30, "location": "Downtown"},
            {"name": "Sushi World", "cuisine": "Japanese", "rating": 4.0, "delivery_time": 25, "location": "Uptown"},
            {"name": "Taco Bell", "cuisine": "Mexican", "rating": 3.5, "delivery_time": 45, "location": "Suburb"},
            {"name": "Burger King", "cuisine": "American", "rating": 4.2, "delivery_time": 35, "location": "City Center"},
            {"name": "Dragon Wok", "cuisine": "Chinese", "rating": 4.8, "delivery_time": 20, "location": "Midtown"},
        ]
        self.restaurant_filter = RestaurantFilter(restaurants_data)
        ...
        # Results Treeview
        self.results_tree = ttk.Treeview(self, columns=("cuisine", "location", "rating", "delivery_time"), show="headings")
        self.results_tree.heading("cuisine", text="Cuisine")
        self.results_tree.heading("location", text="Location")
        self.results_tree.heading("rating", text="Rating")
        self.results_tree.heading("delivery_time", text="Delivery Time (min)")
        self.results_tree.pack(pady=10, fill="x")
    ...
    def search_restaurants(self):
        ...
        # Collect filter values from user inputs
        cuisine = self.cuisine_var.get().strip()
        try:
            max_rating = float(self.max_rating_var.get().strip()) if self.max_rating_var.get().strip() else 5
            max_delivery_time = int(self.max_delivery_var.get().strip()) if self.max_delivery_var.get().strip() else 60
        ...
        for r in filtered_restaurants:
            self.results_tree.insert("", "end", values=(r["cuisine"], r["location"], r["rating"], r["delivery_time"]))

    def view_all_restaurants(self):
        self.results_tree.delete(*self.results_tree.get_children())
        # In the actual app, use self.database.get_restaurants() for live data
        results = self.database.get_restaurants()
        for r in results:
            self.results_tree.insert("", "end", values=(r["cuisine"], r["location"], r["rating"], r["delivery_time"]))
